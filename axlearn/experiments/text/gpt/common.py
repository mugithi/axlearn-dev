    ...
    def config_fn() -> InstantiableConfig:
        ...
        if not checkpointer:
            ...
            ckpt_config.save_policy = config_for_function(every_n_steps_and_last_policy).set(
                n=50, # Desired GCS interval
                max_step=max_step,
            )
            # Set local save policy
            ckpt_config.local_save_policy = config_for_function(every_n_steps_and_last_policy).set(
                n=15, # Desired local interval
                max_step=max_step,
            )
            ckpt_config.local_dir = "/host-tmp/checkpoints" # Consistent with run_orbax.sh
            ckpt_config.keep_every_n_steps = min(max_step, keep_every_n_steps)
            ckpt_config.keep_last_n = 3
            # Ensure replica_axis_index is set if required by your OrbaxEmergencyCheckpointer version
            # Defaulting to 1 as seen in some configs, adjust if necessary.
            if hasattr(ckpt_config, 'replica_axis_index'):
