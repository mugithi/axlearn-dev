...
# Install specific libtpu version for hang simulation support
RUN pip install --no-cache-dir --force-reinstall libtpu==0.0.14
COPY . .
# RUN pip install -f --force-reinstall git+https://github.com/google/orbax.git@test_765598157#subdirectory=checkpoint
# Orbax version is now managed by pyproject.toml in the [orbax] extras section
